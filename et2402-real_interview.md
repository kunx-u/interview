...
16. Java保证线程安全的关键字?
    `synchronized` : 用于修饰方法或代码块,同一时刻只有一个线程能够访问被修饰的方法或代码块,从而保证了并发访问的同步性和数据一致性; 适用于多个线程访问共享资源的情况
    `volatile` : 主要用于修饰变量,保证了变量的可见性和禁止指令重排序优化.当一个变量被声明为`volatile`后,它的值的改变对于其他线程是立即可见的,避免了线程因本地缓存而导致的对变量值的不一致认知
    `final` : 用于修饰类,方法和变量.修饰类时,表示该类不可被继承;修饰方法时,该方法不能被重写;修饰变量时,变量一旦初始化就不可再修改,对于基本数据类型变量,其值不可变,对于引用类型变量,其引用不可变,但所指对象的内容可变

18. Synchronized怎么保证线程安全，持有的锁对象是什么?
    **如何保证线程安全** : 当一个线程访问被 synchronized 修饰的方法或代码块时,它会先获取对应的锁,只有获取到锁的线程才能执行被保护的代码,其他线程若也想执行这段代码,就必须等待当前持有锁的线程释放锁,从而保证了同一时刻只有一个线程能访问和修改共享资源,避免了多个线程同时操作共享资源导致的数据不一致问题,以此来保证线程安全
    **锁对象** : 1.修饰普通方法:锁对象时调用该方法的实例对象本身 2.修饰静态方法:锁对象时该类的 Class 对象 3.修饰代码块:锁对象由程序员在代码块中显式指定

20. 聊一下对JVM的认识?
    JVM是Java虚拟机,它是Java程序的运行核心
    **功能特性** : 跨平台性:JVM是Java实现"一次编写,到处运行"的关键.它屏蔽了不同操作系统和硬件的差异,使得Java程序只需编译成字节码,就能在任何安装了对应JVM的设备上运行  内存管理:JVM负责管理Java程序运行时的内存,包括自动内存分配和垃圾回收机制  字节码执行:它可以将字节码解释或编译成机器码并执行
    **组成结构** : 1.类加载器子系统 2.运行时数据区 3.执行引擎 4.本地方法接口

22. JVM的内存结构?
    1.`程序计数器` : 一块较小的内存空间,可以看作是当前线程所执行的字节码的行号指示器
    2.`Java虚拟机栈` : 与程序计数器一样,Java虚拟机栈也是线程私有的,它的生命周期与线程相同,描述的是Java方法执行的内存模型,每个方法在执行的同时都会创建一个栈帧,用于存储局部变量,操作数栈,动态连接,方法出口等信息 *特点* : 栈大小可以固定,也可以动态扩展
    3.`本地方法栈` : 与Java虚拟机栈非常相似,区别在于虚拟机栈为Java方法服务,而本地方法栈则是为本地方法服务的. *特点* : 本地方法栈同样是线程私有的,其具体的实现方法和内存大小限制等也与虚拟机栈类似,在某些情况下,本地方法栈和虚拟机栈可能会合并在一起实现
    4.`堆` : 是Java虚拟机所管理的内存中最大的一块, 它是被所有线程共享的一块生存区域,几乎所有的对象实例和数组都在这里分配内存. 主要作用就是存放对象实例,它是Java程序运行时动态分配内存的主要场所 *特点* : 堆内存的大小可以通过参数进行设置,并且它是一个可动态扩展的内存区域,由于堆是所有线程共享的,因此在多线程环境下,对堆内存的访问需要进行同步控制,以避免出现数据不一致等问题
    5.`方法区` : 也是所有线程共享的内存区域,它用于存储已被虚拟机记载的类信息,常量,静态变量,即时编译器编译后的代码等数据 *特点* : 方法区的大小通常也是可以通过参数进行设置的，并且在某些情况下，方法区也可能会被实现为一个可动态扩展的区域。
    6.`运行时常量池` : 运行时常量池是方法区的一部分,主要用于存放编译期生成的各种字面量和符号变量 *特点* : 运行时常量池的大小同样受到方法区大小的限制

24. 堆和栈分别有哪些区域?

25. GC对堆内每一个区域怎么收集，有哪些收集器、算法?

26. 新生代老年代了解过吗?

27. Spring的核心思想?

28. 将bean交给Spring管理的方式?

29. Spring创建对象的过程?

30. Spring为什么是单例的，没有线程安全问题吗?
...
39. Redis怎么保证和数据库的一致性?

40. 项目中的Redis使用的什么数据类型?
